...
export function SkillsSection() {
  ...
  return (
    <Section id="skills" ariaLabel="Technology stack">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <SectionHeading
          eyebrow="Stack"
          title="The tools behind the automations"
          description="Grouped the way the work is actually organised. Where each of these was applied is in the projects above."
        />
      ...
      {groups.length === 0 ? (
        <p className="surface-card mt-10 p-10 text-center text-[0.95rem] text-[var(--text-secondary)]">
          Nothing in the stack matches “{query}”. That does not mean he could not learn it — it
          means it is not something the resume can claim.
        </p>
      ) : (
                ...
                <ul className="mt-4 flex flex-wrap gap-1.5">
                  {matches.map((skill) => (
                    <li
                      key={skill}
                      className={cn(
                        'rounded-[var(--radius-sm)] border px-2.5 py-1 text-[0.76rem] transition-colors duration-[var(--motion-fast)]',
                        query && skill.toLowerCase().includes(query.trim().toLowerCase())
